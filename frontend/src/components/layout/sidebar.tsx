  ...
  return (
    ...
    <div className="w-[280px] min-h-screen bg-[#1f2d3d] text-white flex flex-col justify-between p-4">
      ...
      <div className="text-center mt-8">
        <p className="mb-[10px] text-[#FFF] ">
          Welcome, <span className="text-[#2992dc]">{user?.name.toUpperCase()}</span> ({user?.role?.toLowerCase()})
        </p>
        <button
          onClick={logout}
          className="bg-[#88569d] text-[#FFF] hover:bg-[#462c50] px-[40px] py-[10px] mb-[20px] rounded-[8px] border-none"
        >
          <Link to='/' className='no-underline text-[#FFF]'>Logout</Link>
        </button>
      </div>
    </div>
  </div>
